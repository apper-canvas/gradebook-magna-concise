...
// Attendance bulk selection state
  ...
  const handleBulkAttendanceAction = async (status) => {
    ...
    try {
      const promises = Array.from(selectedAttendanceStudents).map(studentId =>
        studentService.updateStudentAttendance(studentId, selectedDate, status)
      );
      ...
      toast.success(`Marked ${selectedAttendanceStudents.size} students as ${status}`);
    } catch (err) {
  ...
  const handleMarkAllPresent = async () => {
    try {
      const promises = filteredStudents.map(student =>
        studentService.updateStudentAttendance(student.Id, selectedDate, "Present")
      );
  ...
  const handleAttendanceStudentToggle = (studentId) => {
    const newSelected = new Set(selectedAttendanceStudents);
    if (newSelected.has(studentId)) {
      newSelected.delete(studentId);
    } else {
      newSelected.add(studentId);
    }
    setSelectedAttendanceStudents(newSelected);
  };

  const handleSelectAllAttendance = () => {
    if (selectedAttendanceStudents.size === filteredStudents.length) {
      setSelectedAttendanceStudents(new Set());
    } else {
      setSelectedAttendanceStudents(new Set(filteredStudents.map(s => s.Id)));
    }
  };
useEffect(() => {
    loadStudents();
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div className="flex items-center gap-2">
            ...
            {selectedAttendanceStudents.size > 0 && (
              <>
                ...
                <Button
                  onClick={() => setSelectedAttendanceStudents(new Set())}
                  variant="outline"
                  size="sm"
                  className="gap-2"
                >
          ...
          {selectedAttendanceStudents.size > 0 && (
            <Badge variant="secondary">
              {selectedAttendanceStudents.size} students selected
            </Badge>
          )}
        </div>
      </div>
            ...
            <table className="w-full">
                <thead className="bg-muted/50">
                    <tr>
                        <th className="text-left p-4 font-semibold text-muted-foreground">
                            <input
                                type="checkbox"
                                checked={selectedAttendanceStudents.size === filteredStudents.length && filteredStudents.length > 0}
                ...
                <tbody>
                    {filteredStudents.map(student => {
                        ...
                        const isSelected = selectedAttendanceStudents.has(student.Id);

                        return (
                            <motion.tr
                                key={student.Id}
                                className={`border-b hover:bg-muted/30 transition-colors ${isSelected ? "bg-primary-50" : ""}`}
                                initial={{
                                    opacity: 0
                                }}
                                animate={{
                                ...
                                <td className="p-4">
                                    <input
                                        type="checkbox"
                                        checked={isSelected}
                                        onChange={() => handleAttendanceStudentToggle(student.Id)}
                                        className="w-4 h-4 text-primary-600 rounded focus:ring-primary-500" />
                                </td>
                                <td className="p-4">
                                    <div className="flex items-center gap-3">
                                        <div
                                            className="w-10 h-10 bg-gradient-to-br from-primary-500 to-accent-500 rounded-full flex items-center justify-center text-white font-semibold">
  ...
  return (
    ...
    <div
        ...
        <div>
            ...
            <p className="text-slate-600 font-medium mt-2">Manage your students and track their academic progress
            </p>
        </div>
        ...
        <div
            ...
            <div className="flex gap-2">
                <Button
                    ...
                    className="gap-2">
                    <ApperIcon name="Users" size={16} />Roster
                                </Button>
                <Button
                    variant={viewMode === "attendance" ? "default" : "outline"}
                    onClick={() => setViewMode("attendance")}
                    className="gap-2">
                    <ApperIcon name="Calendar" size={16} />Attendance
                                </Button>
                <Button
    ...
    {/* Class Statistics */}
    {viewMode === "roster" && <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <Card
            className="p-6 bg-gradient-to-br from-primary-50 to-primary-100 border-primary-200">
        ...
        <Card
            className="p-6 bg-gradient-to-br from-success-50 to-success-100 border-success-200">
            <div className="flex items-center space-x-3">
        ...
        <Card
            className="p-6 bg-gradient-to-br from-accent-50 to-accent-100 border-accent-200">
            <div className="flex items-center space-x-3">
        ...
        <Card
            className="p-6 bg-gradient-to-br from-warning-50 to-warning-100 border-warning-200">
            <div className="flex items-center space-x-3">
    ...
    <Card>
        ...
        <CardContent>
            {viewMode === "roster" ? <StudentTable
                ...
                selectedStudent={selectedStudent} /> : viewMode === "attendance" ? renderAttendanceView() : viewMode === "assignment" ? <motion.div
                initial={{
                    opacity: 0,
                    y: 20
                }}
                ...
                <Card>
                    <CardHeader>
                        <CardTitle className="flex items-center gap-2">
                            <ApperIcon name="Move" size={20} />Class Assignment
                                              </CardTitle>
                    </CardHeader>
                    <CardContent>
                ...
                <Card>
                    <CardHeader>
                        <CardTitle className="flex items-center gap-2">
                            <ApperIcon name="BookOpen" size={20} />Available Classes
                                                {selectedStudents.size > 0 && <Badge variant="secondary">
                    ...
                    <CardContent>
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            {availableClasses.map(classItem => <motion.div
                                key={classItem.Id}
                                className={`border-2 border-dashed rounded-lg p-4 transition-all duration-200 ${dragOverClass === classItem.Id ? "border-primary-400 bg-primary-50" : "border-slate-300 hover:border-slate-400"}`}
                                ...
                                <div className="flex items-center gap-3 mb-3">
                                    <div
                                        className={`w-4 h-4 rounded-full ${classItem.color === "primary" ? "bg-primary-500" : classItem.color === "accent" ? "bg-accent-500" : classItem.color === "success" ? "bg-success-500" : classItem.color === "warning" ? "bg-warning-500" : "bg-error-500"}`} />
                                    <div>
                                ...
                                {selectedStudents.size > 0 && <Button
                                    size="sm"
                                    variant="outline"
                                    onClick={() => handleBulkAssignToClass(classItem.Id)}
                                    disabled={assignmentLoading}
                                    className="w-full gap-2">
                                    {assignmentLoading ? <ApperIcon name="Loader2" size={14} className="animate-spin" /> : <ApperIcon name="UserPlus" size={14} />}Assign {selectedStudents.size}Student{selectedStudents.size !== 1 ? "s" : ""}
                                </Button>}
                            </motion.div>)}
                        </div>
                    </CardContent>
                ...
                <Card>
                    <CardHeader>
                        <div className="flex items-center justify-between">
                            <CardTitle className="flex items-center gap-2">
                                <ApperIcon name="Users" size={20} />Student Roster
                                                    </CardTitle>
                            {selectedStudents.size > 0 && <Button
                                ...
                                size="sm"
                                className="gap-2">
                                <ApperIcon name="X" size={14} />Clear Selection
                                                      </Button>}
                        </div>
                    </CardHeader>
                    <CardContent>
                ...
                <Card>
                    <CardHeader>
                        <CardTitle className="flex items-center gap-2">
                            <ApperIcon name="Edit3" size={20} />Assignment Details
                                              </CardTitle>
                    </CardHeader>
                    <CardContent className="space-y-4">
                        ...
                        {bulkGradeData.maxScore && selectedStudents.size > 0 && <div className="mt-4 p-4 bg-slate-50 rounded-lg border">
                            <div className="flex flex-col sm:flex-row sm:items-center gap-4">
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {["A", "B", "C", "D", "F"].map(grade => <Button
                                        ...
                                        className="gap-1">
                                        {grade}
                                        <span className="text-xs text-slate-500">({grade === "A" ? "90%" : grade === "B" ? "80%" : grade === "C" ? "70%" : grade === "D" ? "60%" : "0%"})
                                                                          </span>
                                    </Button>)}
                                    <Button
                                        variant="outline"
                                        ...
                                        className="gap-1 text-success-600 hover:text-success-700 border-success-200 hover:border-success-300">Full Credit
                                    ...
                                    <Button
                                        ...
                                        className="gap-1 text-warning-600 hover:text-warning-700 border-warning-200 hover:border-warning-300">Incomplete
                ...
                <Card>
                    <CardHeader>
                        <div className="flex items-center justify-between">
                            <CardTitle className="flex items-center gap-2">
                                <ApperIcon name="Users" size={20} />Student Grades
                                                      {selectedStudents.size > 0 && <Badge variant="secondary">
                            ...
                            <Button
                                ...
                                className="gap-2">
                                {bulkLoading ? <ApperIcon name="Loader2" size={16} className="animate-spin" /> : <ApperIcon name="Save" size={16} />}Save Grades
                                                    </Button>
                        </div>
                    </CardHeader>
                    <CardContent>
                                ...
                                <table className="min-w-full divide-y divide-slate-200">
                                    <thead className="bg-gradient-to-r from-slate-50 to-slate-100">
                                        <tr>
                                            ...
                                            <th
                                                className="px-6 py-4 text-left text-xs font-bold text-slate-700 uppercase tracking-wider">Student
                                                                            </th>
                                            <th
                                                className="px-6 py-4 text-left text-xs font-bold text-slate-700 uppercase tracking-wider">Current Average
                                                                            </th>
                                            <th
                                                className="px-6 py-4 text-left text-xs font-bold text-slate-700 uppercase tracking-wider">Score
                                                                            </th>
                                        </tr>
                                    </thead>
                                    <tbody className="bg-white divide-y divide-slate-200">
                                        {filteredStudents.map(student => <tr
                                            key={student.Id}
                                            className={`hover:bg-slate-50 transition-colors ${selectedStudents.has(student.Id) ? "bg-primary-50" : ""}`}>
                                            ...
                                            <td className="px-6 py-4">
                                                <Badge
                                                    variant={student.gradeAverage >= 90 ? "success" : student.gradeAverage >= 80 ? "secondary" : student.gradeAverage >= 70 ? "warning" : "destructive"}>
                                                    {student.gradeAverage.toFixed(1)}%
                                                                                    </Badge>
                                            </td>
                                            <td className="px-6 py-4">
                                                {selectedStudents.has(student.Id) ? <div className="space-y-2">
                                                    <Input
                                                        type="number"
                                                        ...
                                                        step="0.1"
                                                        className="w-24" />
                                                    {bulkGradeData.maxScore && <div className="flex flex-wrap gap-1">
                                                        {["A", "B", "C", "D", "F"].map(grade => <Button
                                                            key={grade}
                                                            variant="outline"
                                                            size="sm"
                                                        ...
                                                        <Button
                                                            ...
                                                            className="h-6 px-2 text-xs font-medium text-success-600 hover:text-success-700">Full
                                                                                                    </Button>
                                                        <Button
                                                            variant="outline"
                                                            size="sm"
                                                            onClick={() => handleQuickGrade(student.Id, "Incomplete")}
                                                            className="h-6 px-2 text-xs font-medium text-warning-600 hover:text-warning-700">Inc
                                                                                                    </Button>
                                                    </div>}
                                                </div> : <span className="text-slate-400 text-sm">Select to enter score</span>}
                                            </td>
                                        </tr>)}
                                    </tbody>
                                </table>
